...
class AdvancedPostureAnalyzer:
    ...
    def calculate_angle(self, v1, v2):
        """Calculate angle between two vectors using numpy's dot product"""
        dot_product = np.dot(v1, v2)
        norms = np.sqrt(np.sum(v1**2)) * np.sqrt(np.sum(v2**2))
    ...
    def calculate_posture_metrics(self, body_points, chair_bbox):
        ...
        spine_angle = self.calculate_angle(spine_vector, vertical)
        ...
        distance_cm = np.clip(distance_cm, 0, 50)  # Cap at 50cm
        
        return {
            'spine_angle': float(spine_angle),  # Convert to native Python float
            'distance_cm': float(distance_cm),   # Convert to native Python float
            'is_side_view': bool(is_side_view)   # Convert to native Python bool
        }
    ...
    def analyze_image(self, image):
        ...
        try:
            ...
            if chair_bbox is None:
                return None, "No chair detected in image", None, None
            
            # Detect body landmarks
            body_points = self.detect_body_landmarks(image)
            if body_points is None:
                return None, "Could not detect body landmarks", None, None
            ...
            return None, f"Error analyzing image: {str(e)}", None, None
